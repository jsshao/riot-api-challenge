...
def load_champions():
    ...
    with open('champion' + version + '.json', 'r') as champs:
        ...
        for key in data["data"]:
            ...
            champion_id_to_name[id] = "AP " + champ
            ...
            with open(f) as json_input:
                ...
                for player in data["participants"]:
                    ...
                    for j in range(7):
                        ...
                        if item == 0:
                            continue
                        total_cost += item_data["data"][str(item)]["gold"]["total"]
                        if "FlatMagicDamageMod" in item_data["data"][str(item)]["stats"]:
                            ap_item_cost += item_data["data"][str(item)]["gold"]["total"]
                    if total_cost == 0 or float(ap_item_cost) / total_cost < 0.4:
                        continue
...
def main():
    ...
    with open('ap_build_tree_%s.json'%version, 'w') as f:
        store_tree(tree_by_champ, f)

    with open('ap_item_%s.csv'%version,'w') as f:
    ...
    with open('ap_champ_%s.csv'%version,'w') as f:
        f.write('name,id,popularity,winrate\n')
    ...
    with open('ap_champ_items_%s.csv'%version,'w') as f:
        f.write("name,id,")
    ...
    with open('ap_champ_build_%s.csv' % version, 'w') as f:
